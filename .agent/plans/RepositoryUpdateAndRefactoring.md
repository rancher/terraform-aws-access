...
2.  **CSpell Configurations (`cspell.json` & `custom_words.txt`)**:
    *   Create `cspell.json` and a `custom_words.txt` file in the root of the repository containing expected technical terms (e.g., `authconfig`, `kubeconfig`, `nutanix`, `variablize`).
...
3.  **Checkout & Script Externalization (`.github/workflows/scripts/`)**:
    *   Replace all uses of `actions/checkout` with the following:
        ```yaml
          - name: Checkout
            shell: bash
            run: |
...
**Objective**: Standardize validations and ensure robust path resolution in all Terraform files.
...
## Phase 4: Examples Directory (`examples/`)
**Objective**: Standardize the test fixtures and usage examples.

1.  **Refactor Test Examples**:
    *   Ensure all modules/examples provision domains utilizing the dynamic test `$IDENTIFIER` so they can be traced and destroyed.

---

## Phase 5: Go Testing Suite Overhaul (`./test/tests/` -> `./test/`)
...
2.  **Dependency Updates**:
    *   Bump `go.mod` (in all test modules) to Go version `1.26.0`.
