...
* **Checkout Method:** When running inside the nix ci-image container, replace all uses of `actions/checkout` with the following block to avoid permissions issues:
  ```yaml
    - name: Checkout
      shell: bash
      run: |
  ...
  ```
  *Exception:* When a job runs directly on the runner without the container (e.g., `release.yaml`), use the standard `actions/checkout` action (pinned by SHA).
